...
function LineChart({ index }: { index: number }) {
  ...
  return <svg className="renstra-chart-svg" viewBox="0 0 640 280" role="img" aria-label={`Grafik ${iku[index].name}, target 2025 sampai 2029`}>
    ...
    {data.map((v, i) => <g key={i}><circle cx={x(i)} cy={y(v)} r="5" className="chart-dot"/><text x={x(i)} y={y(v)-12} textAnchor="middle" className="chart-value">{format(v)}</text></g>)}
  </svg>;
}
function FundingChart({ hidden }: { hidden: Set<number> }) {
  ...
  return <svg className="renstra-chart-svg funding-svg" viewBox="0 0 640 320" role="img" aria-label="Grafik batang bertumpuk pagu indikatif per unit dan tahun dalam miliar rupiah">
    ...
    {years.map((yr, j) => { let acc = 0; return <g key={yr}>{units.map((u, i) => { if (hidden.has(i)) return null; const v=u.values[j], top=acc+v; const el=<rect key={u.key} x={x(j)-32} y={y(top)} width="64" height={Math.max(0,y(acc)-y(top))} fill={colors[i]}><title>{`${u.name} ${yr}: Rp${format(v.toFixed(2))} M`}</title></rect>; acc=top; return el; })}<text x={x(j)} y={y(acc)-8} textAnchor="middle" className="chart-value">{format(acc.toFixed(1))}</text><text x={x(j)} y="300" textAnchor="middle">{yr}</text></g>})}
  </svg>;
}
function AccordionSection({ section }: { section: typeof accordionSections[number] }) { return <section id={section.id}><div className="renstra-wrap"><h2>{section.title}</h2><p className="sub">{section.intro}</p>{section.items.map(([title, body]) => <details key={title}><summary>{title}</summary><p>{body}</p></details>)}</div></section>; }

export default function RenstraPmp() {
  const [theme, setTheme] = useState<"light"|"dark">("light"); const [ikuIndex, setIkuIndex] = useState(0); const [hidden, setHidden] = useState<Set<number>>(new Set()); const [focus, setFocus] = useState<string | null>(null); const [active, setActive] = useState("konteks"); const [progress, setProgress] = useState(0);
  ...
  return <div className="renstra-page" data-theme={theme}>
    ...
    <header className="renstra-hero"><div className="renstra-wrap"><div className="kicker">{renstraMeta.kicker}</div><h1>{renstraMeta.title}</h1><p>{renstraMeta.intro}</p><div className="goals">{renstraMeta.goals.map((goal,i)=><div className="goal" key={goal}><b>Tujuan {i+1}</b>{goal}</div>)}</div><div className="facts">{renstraMeta.facts.map(f=><div key={f}><strong>{f.split(" ")[0]}</strong>{f.substring(f.indexOf(" ")+1)}</div>)}</div></div></header>
    ...
    <section id="organisasi"><div className="renstra-wrap"><h2>Enam unit, satu mandat makro</h2><p className="sub">Berdasarkan Permen PPN/Kepala Bappenas No. 2 Tahun 2025. Pilih unit untuk menyorotnya di grafik pendanaan.</p><div className="units">{units.map((u,i)=><button className="unit" key={u.key} onClick={()=>{toggleUnit(i);document.getElementById("dana")?.scrollIntoView({behavior:"smooth"})}}><span className="sw" style={{background:colors[i]}}/><b>{u.name}</b><small>{u.description}</small></button>)}</div><h3>Kebutuhan pegawai belum terpenuhi</h3><p className="sub">Analisis Beban Kerja menunjukkan kekurangan 178 pegawai. Jabatan perencana paling terdampak.</p><div className="sdm">{sdm.map(row=><div className="sdm-row" key={row.label}><span>{row.label}</span><div className="track"><div className="fill" style={{width:`${row.value/96*100}%`}}/></div><b>{row.value}</b></div>)}</div><p className="src">Sumber: Renstra hlm. 21–22.</p></div></section>
    <section id="pohon"><div className="renstra-wrap"><h2>Pohon kinerja</h2><p className="sub">Ketuk kotak mana pun untuk melihat rantai sasaran dan indikator terkait.</p><div className={`tree ${focus?"has-focus":""}`}>{treeColumns.map(col=><div className="tree-col" key={col.title}><h4>{col.title}</h4>{col.nodes.map(node=><button key={node.id} className={`node ${related.has(node.id)?"hit":""}`} onClick={()=>setFocus(focus===node.id?null:node.id)} aria-pressed={focus===node.id}>{node.text}</button>)}</div>)}</div><p className="src">Sumber: Tabel 2.1, Renstra hlm. 36–37.</p></div></section>
    {accordionSections.slice(4).map(s=><AccordionSection key={s.id} section={s}/>)}
    <section id="target"><div className="renstra-wrap"><h2>Target IKU 2025–2029</h2><p className="sub">Empat IKU yang menjadi acuan Perjanjian Kinerja tahunan. Pilih indikator.</p><div className="tabs" role="group" aria-label="Pilih IKU">{iku.map((item,i)=><button key={item.name} className="tab" onClick={()=>setIkuIndex(i)} aria-pressed={ikuIndex===i}>{item.name}</button>)}</div><div className="chart"><LineChart index={ikuIndex}/></div><p className="sub">{iku[ikuIndex].note}</p><p className="src">Sumber: Tabel 3.3, Renstra hlm. 124–125.</p></div></section>
    <section id="dana"><div className="renstra-wrap"><h2>Kerangka pendanaan</h2><p className="sub">Pagu indikatif per unit kerja (Rupiah Murni), dalam miliar rupiah. Ketuk nama unit untuk menyembunyikan atau menampilkan.</p><div className="chart"><FundingChart hidden={hidden}/></div><div className="legend">{units.map((u,i)=><button key={u.key} onClick={()=>toggleUnit(i)} aria-pressed={!hidden.has(i)}><i style={{background:colors[i]}}/>{u.name.replace("Dit. ","")}</button>)}</div><p className="note"><b>Catatan verifikasi:</b> kolom “Total” per unit di tabel sumber tidak sama dengan jumlah lima tahunnya (misalnya Sekretariat Deputi tertulis Rp33,1 M, sedangkan penjumlahan per tahun sekitar Rp80,2 M). Grafik ini memakai angka per tahun. Total penjumlahan tahunan adalah Rp329,6 M, sedangkan tabel mencantumkan Rp383,6 M. Angka tidak dikoreksi secara diam-diam.</p><p className="src">Sumber: Bab 3.4, Renstra hlm. 132–144.</p></div></section>
