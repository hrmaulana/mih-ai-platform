...
function SourceTableView({ table }: { table: SourceTable }) {
  return <figure className="source-table-figure">
    ...
    <div className="source-table-scroll">
      <table className="source-table"><thead><tr>{table.columns.map((column) => <th scope="col" key={column}>{column}</th>)}</tr></thead>
        <tbody>{table.rows.map((row, rowIndex) => <tr key={`${table.id}-${rowIndex}`} style={{ "--row-index": rowIndex } as React.CSSProperties}>{row.map((cell, cellIndex) => cellIndex === 0 && table.id !== "1.2" && table.id !== "3.3" && table.id !== "3.11" ? <th scope="row" key={cellIndex}>{cell}</th> : <td key={cellIndex}>{cell}</td>)}</tr>)}</tbody>
      </table>
    </div>
  </figure>;
}
...
function ProcessMap() { return <div className="process-map">{processMap.map((item) => <details key={item.id} className="process-card"><summary><span>{item.unit}</span><small>PDF hlm. {item.pages}</small></summary><div className="process-body"><p>{item.summary}</p><ol>{item.steps.map((step) => <li key={step}>{step}</li>)}</ol></div></details>)}</div>; }
...
export default function RenstraPmp() {
  ...
  return <div className="renstra-page" data-theme={theme}>
    ...
    {accordionSections.slice(4).map(s=>s.id === "proses-bisnis" ? <section id={s.id} key={s.id}><Reveal><div className="renstra-wrap"><h2>{s.title}</h2><p className="sub">Peta digital ini merangkum narasi dan unit yang terpetakan pada PDF Renstra hlm. 70–117. Buka unit/fungsi untuk melihat cakupan dan rujukan halaman.</p><ProcessMap/><p className="src">Sumber: Renstra Deputi Bidang PMP 2025–2029, Bab 2.6, hlm. 70–117 (Gambar 2.6–2.12). Detail visual diagram tetap dirujuk ke PDF sumber.</p></div></Reveal></section> : <AccordionSection key={s.id} section={s}/>)}
