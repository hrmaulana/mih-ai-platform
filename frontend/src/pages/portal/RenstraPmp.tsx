  ...
  return <figure className="source-table-figure">
    ...
    <div className="source-table-scroll">
      <table className="source-table"><thead><tr>{table.columns.map((column) => <th scope="col" key={column}>{column}</th>)}</tr></thead>
        <tbody>{table.rows.map((row, rowIndex) => <tr key={`${table.id}-${rowIndex}`}>{row.map((cell, cellIndex) => cellIndex === 0 && table.id !== "1.2" && table.id !== "3.3" && table.id !== "3.11" ? <th scope="row" key={cellIndex}>{cell}</th> : <td key={cellIndex}>{cell}</td>)}</tr>)}</tbody>
      </table>
    </div>
  </figure>;
}
...
function AccordionSection({ section }: { section: typeof accordionSections[number] }) {
  const tableIds: Record<string, string[]> = { dinamika: ["1.2"], regulasi: ["2.9", "2.10"] };
