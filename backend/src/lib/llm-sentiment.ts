...
/**
 * Analisis sentimen berita menggunakan DeepSeek v4 flash via OpenRouter.
 * Gunakan hanya untuk kasus borderline (|lex_score| < 0.3).
 */
...
{"sentimen": "positif|negatif|netral", "score": -1.0..1.0, "alasan": "...", "keyword": "...", "ringkasan": "..."}`
  ...
  try {
    ...
    if (!response.ok) {
      console.warn(
        `[llm-sentiment] OpenRouter returned ${response.status}`,
      )
      return { score: 0, keyword: "", ringkasan: "" }
    }
